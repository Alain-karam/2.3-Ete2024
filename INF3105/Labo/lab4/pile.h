...
template<class T>
Pile<T>::Pile(const Pile<T>& aCopier):sommet(nullptr) {
  Cellule* copieCourante = aCopier.sommet;
  Cellule** destination = &sommet;

  while (copieCourante) {
    (*destination) = new Cellule(copieCourante->contenu, nullptr);
  }
